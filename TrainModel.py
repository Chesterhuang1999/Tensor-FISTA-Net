...
def train_model(sess, saver, costAll, optmAll, Yinput, prediction, trainLabel, trainPhi, Xinput, Xoutput):
    ...
    # pre-process
    randAll = np.random.permutation(nTrainData // batchSize)     # disorganize order
    trainPart = randAll[ :int(trainScale*len(randAll))]          # training part indices
    validatingPart = randAll[int(trainScale*len(randAll)): ]     # validating part indices
    ...
    for epoch in range(1, nEpoch + 1):  # all epoch
        ...
        for batchi in validatingPart:
            ...
            # batch validating reconstruction
            start = time()
            result = sess.run(prediction[-1], feed_dict = feedDict)            
            end = time()

            # batch validating results
